...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		{
			if(i==carr.length -1 && Character.toLowerCase(carr[i]) == 'e' && newsyll && numsyll>0)
			{
				numsyll--;
			}
			if(newsyll && vowel.indexOf(Character.toLowerCase(carr[i])) >= 0)
			{
				newsyll = false;
				numsyll++;
			}
			else if(vowel.indexOf(Character.toLowerCase(carr[i])) < 0)
			{
				newsyll = true;
			}
		}
	...
	public double getFleschScore()
	{
		...
		int syll = this.getNumSyllables();
		int word =this.getNumWords();
		int sen =this.getNumSentences();
		double Flesh = 206.835 - 1.015*((double)word/sen) - 84.6*((double)syll/word);
	    return Flesh;
	}
